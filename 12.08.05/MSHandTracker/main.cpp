	...
	while((mOpenNIChecker = mContext.WaitOneUpdateAll(mDepthGenerator)) == XN_STATUS_OK && gRun)
	{
		...
		if(g_SessionState == IN_SESSION)
		{
			...
			if(depthMap)
			{
				...
				//mUserGenerator.GetUsers(users, maxUsers);


				//mUserGenerator.GetUserPixels(1, smd);
				//const XnLabel *label = smd.Data();
				...
				for(unsigned int i = 0 ; i < dmd.FullYRes() ; i++)
				{
					for(unsigned int j = 0 ; j < dmd.FullXRes() ; j++)
					{
						...
						//image->imageData[i * dmd.FullXRes() +j] *= (*(smd.Data() + i * dmd.FullXRes() +j) == 1)?255:0;
						/*if((int)*(label) == (int)(users[0]))
						{
							image->imageData[i * dmd.FullXRes() +j] = 127;
						}
						else
						{
							image->imageData[i * dmd.FullXRes() +j] = 0;
						}
						label++;*/
					}
				}
				...
				//cvFloodFill(image, seed, cvScalar(255.), cvScalar(10.), cvScalar(10.), NULL, CV_FLOODFILL_FIXED_RANGE);
				...
				/*char filename[128];
				sprintf(filename, "C:\\Users\\Mirou\\Documents\\GitHub\\amir2\\depthmap_%3d.png", count);
